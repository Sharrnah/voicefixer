...
class VoiceFixer(nn.Module):
    ...
    def _load_wav(self, path, sample_rate, dtype=np.float32, threshold=0.95):
        wav_10k, _ = librosa.load(path, sr=sample_rate, dtype=dtype)
        return wav_10k
...
#        return tensor2numpy(out.squeeze(0))
    ...
    def restore_inmem(self, wav_10k, cuda=False, mode=0, sample_rate=44100, your_vocoder_func=None):
        ...
        break_point = 0
        #while break_point < wav_10k.shape[0]+seg_length:
        while break_point < wav_10k.shape[0]:
            #segment = wav_10k[break_point-seg_length:break_point]
            segment = wav_10k[break_point:break_point+seg_length]
            if (mode == 1):
                segment = self.remove_higher_frequency(segment)
            sp,mel_noisy = self._pre(self._model, segment, cuda)
            out_model = self._model(sp, mel_noisy)
    ...
    #def restore(self, input, output, cuda=False, mode=0, input_sample_rate=44100, dtype=np.float32, format='RAW', subtype='PCM_16', your_vocoder_func=None):
    def restore(self, input, output, cuda=False, mode=0, input_sample_rate=44100, dtype=np.float32, your_vocoder_func=None):
        wav_10k = self._load_wav(input, sample_rate=input_sample_rate, dtype=dtype)
        out_np_wav = self.restore_inmem(wav_10k, cuda=cuda, mode=mode, sample_rate=input_sample_rate, your_vocoder_func=your_vocoder_func)
        save_wave(out_np_wav,fname=output,sample_rate=input_sample_rate)
        #save_wave(out_np_wav,fname=output,sample_rate=input_sample_rate, format=format, subtype=subtype)

    def restore_inmem_resample(self, input, cuda=False, mode=0, input_sample_rate=44100, your_vocoder_func=None):
        audio_44k = input
        if input_sample_rate != 44100:
            audio_44k = librosa.resample(y=input, orig_sr=input_sample_rate, target_sr=44100)
        return self.restore_inmem(audio_44k, cuda=cuda, mode=mode, your_vocoder_func=your_vocoder_func)
